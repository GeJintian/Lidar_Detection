...
public:
    ...
    void object_detection(const pcl::PointCloud<PointT> &cloud_in, pcl::PointCloud<PointT> &cloud_object);
...
private:
    ...
    rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr pub_revert_pc, pub_reject_pc, pub_normal, pub_noise, pub_vertical, pub_cloud, pub_ground, pub_non_ground, pub_curbside, pub_object;
...
template<typename PointT> inline
void PatchWorkpp<PointT>::object_detection(const pcl::PointCloud<PointT> &cloud_in, pcl::PointCloud<PointT> &cloud_object){
    cloud_object.clear();
    // pcl::PointCloud<PointT> cloud_filtered;
    ...
    for (const auto& point : cloud_in) {
    if (point.z >= Z_LOWER_BOUND && point.z <= Z_UPPER_BOUND && point.y*point.y+point.x*point.x > SELF_BOUND*SELF_BOUND && point.x > -50 && point.x < 50 && point.y > -50 && point.y < 50) {
        cloud_temp.push_back(point);
    }
    }
...
void PatchWorkpp<PointT>::callbackCloud(const sensor_msgs::msg::PointCloud2::ConstSharedPtr &cloud_msg)
{
    ...
    object_detection(pc_non_ground, pc_object);
