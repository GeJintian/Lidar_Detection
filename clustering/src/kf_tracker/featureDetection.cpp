...
vector<Vec2f> featureDetection::houghTransform(){
	...
	cvtColor(_detectedEdges,_detectedEdgesRGB, COLOR_GRAY2BGR); // converting to RGB
	...
	if (_lines.size() > 1){
		...
		kmeans(samples, 2, labels, TermCriteria(TermCriteria::MAX_ITER|TermCriteria::EPS, 1000, 0.001), 5, KMEANS_PP_CENTERS, centers );
...
Mat featureDetection::lineItr(Mat img, vector<Vec2f> lines, string name){
	...
	Mat imgRGB;
	cvtColor(img,imgRGB,COLOR_GRAY2RGB); // converting the image to RGB for display
