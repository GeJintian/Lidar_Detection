...
void KalmanFilterTracking::KFT(const std::vector<RadarClusterProperties> &property_set, vector<*KalmanFilter> &kf_collection) {

  // First predict, to update the internal statePre variable
  std::vector<cv::Mat> pred;
  for(int i = 0; i < kf_collection.size(); i++){
    pred.push_back(kf_collection[i]->prediction());
  }
...
void KalmanFilterTracking::tracking_cb(){
    if (firstFrame) {
